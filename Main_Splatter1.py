...
# main argument
parser.add_argument('--datasetName', type=str, default='Splatter1/truecounts.csv',
...
parser.add_argument('--L2Para', type=float, default=1.0,
...
recon_df = pd.DataFrame(data_recon)
recon_df.to_csv(args.outputDir + 'Splatter1_recon.csv', index=None, header=None)



## Step5: evaluation
print('dropout data PCCs: {:.4f}, imputed data PCCs: {:.4f}'.
      format(pearson_corr(data_noise[index1, index2], data[index1, index2]),
             pearson_corr(data_recon[index1, index2], data[index1, index2])))
print('dropout data L1: {:.4f}, imputed data L1: {:.4f}'.
      format(l1_distance(data_noise[index1, index2], data[index1, index2]),
             l1_distance(data_recon[index1, index2], data[index1, index2])))
